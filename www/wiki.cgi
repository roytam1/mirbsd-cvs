#!/usr/bin/perl
# $MirOS: www/wiki.cgi,v 1.3 2005/03/03 16:50:24 tg Exp $
#
# MirOS Wiki
...
# The copyright holders for UseModWiki also permit you to redistribute
# this work under a later version of the GNU GPL, but this is not true
# for MirOS Wiki. If you want such a permission, contact the copyright
# holders for MirWiki as shown above.
#
# The GNU GPL version 2 can be found at /usr/share/doc/legal/COPYING-2
# or (usually) at the page "GnuGPL" within the Wiki.
...
use vars qw(@RcDays @HtmlPairs @HtmlSingle
  $IsoFS $AllowHiBit $forceHTTPS $forceSNAME $forceSPORT $forceOn
  $SitePersist $useCrypt $ConfDir
...
# == Configuration =====================================================
$DataDir     = "/var/www/users/MirBSD/wikidata";	# Main wiki directory
$ConfDir     = "/var/www/htdocs/MirBSD";		# wiki config dir
$UseConfig   = 1;       # 1 = use config file,    0 = do not look for config
$ConfigFile  = "$ConfDir/wiki.cfg";		# Configuration file
$IsoFS       = 1;       # 1 = usemod default $FS, 0 = ISO 646 $FS
$AllowHiBit  = 1;       # 1 = allow 8bit,         0 = strip to 7bit
$forceOn     = 0;	# 1 = use MirOS specific server force
$SitePersist = "";	# set to persistent site link
$useCrypt    = 0;       # use unix-style crypt password hashes
...
#       $ConfigError = T('Unknown Error (no error text)');
  ...
  if ($forceOn) {
    ...
    if ($ENV{SERVER_NAME} != $forceSNAME) {
      $forceFailed = 1;
    }
    if ($ENV{SERVER_PORT} != $forceSPORT) {
      $forceFailed = 1;
    }
    $uri .= $forceSNAME . ":" . $forceSPORT . $ENV{REQUEST_URI};
    if ($forceFailed) {
      $localq = new CGI;
      print $localq->redirect(-uri=>$uri);
    }
  }
  &InitLinkPatterns();
  if (!&DoCacheBrowse()) {
...
# == MirOS specific code (plaintext passwords are EVIL) ================
sub mirosCryptSalted {
  my ($plain, $salt) = @_;
  if ($useCrypt) {
...
sub mirosCheckPW {
  ...
  if ($useCrypt) {
    return (&mirosCryptSalted($password, $crypted) eq $crypted) ? 1 : 0;
  } else {
    return ($password eq $crypted) ? 1 : 0;
  }
}
...
sub mirosCryptPW {
  ...
  if ($useCrypt) {
    ...
    return &mirosCryptSalted($plain, $salt);
  } else {
    return $plain;
  }
}
...
sub GetAuthorLink {
  ...
  if (($uid > 0) && ($userName ne "")) {
    $html = &ScriptLinkTitle($userName, $userNameShow . " (".$uid.")",
...
sub GetHeader {
  ...
  if ($SitePersist ne "") {
    $result .= "<a href=\"$SitePersist" . $id . "\">PersistentLink</a> | ";
  }
  $result .= &ScriptLink("action=login", "LogIn") . " | ";
...
sub mirosCheckUserIs {
  my ($userPassword, $passwordList) = @_;

  foreach (split(/\s+/, $passwordList)) {
    next  if ($_ eq "");
...
sub mirosEncodeUserIs {
  my ($userPassword, $passwordList) = @_;

  foreach (split(/\s+/, $passwordList)) {
    next  if ($_ eq "");
    return $_  if (&mirosCheckPW($userPassword, $_));
  }
  return mirosCryptPW($userPassword);
}
...
sub UserIsAdmin {
  ...
  return &mirosCheckUserIs($userPassword, $AdminPass);
}

sub UserIsEditor {
  my (@pwlist, $userPassword);

  return 0  if ($AdminPass eq "");		# Admin includes editor
  $userPassword = &GetParam("adminpw", "");
  return 0  if ($userPassword eq "");
  return 1  if (&mirosCheckUserIs($userPassword, $AdminPass));
  return 0  if ($EditPass eq "");
  return &mirosCheckUserIs($userPassword, $EditPass);
}
...
sub WriteStringToFile {
  ...
  if (!$AllowHiBit) {
    # We won't allow control and high-bit characters to fuck up everything
    $string =~ tr/\x80-\xFF/e ,f,.tT\^×S<ö Z  ''""·\-\-\~Ts>ö zÿ !cLxY|$_ca<\-\-R_o+23\'mp.,1o>123?AAAAAAACEEEEIIIIDNOOOOO*0UUUUYPBaaaaaaaceeeeiiiidnooooo:0uuuuypy/;
  }
...
sub DoEdit {
  ...
  if (-f &GetLockedPageFile($id)) {
    $extraHeader = ' <strong style="color:red;">(locked)</strong>';
  }
  ...
  print &GetHeader('', &QuoteHtml($header) . $extraHeader, '');
  if ($revision ne '') {
    print "\n<b>"
...
sub DoEditPrefs {
  ...
  print ' - Remember <b>this number</b>, not the UserName, to login!';
  print '<br />' . T('UserName:') . ' ', &GetFormText('username', "", 20, 50);
  print ' ' . T('(blank to remove, or valid page name)');
  if ($useCrypt) {
    print '<br />Notice: UNIX&reg;-style crypt password hashes are enabled.';
  } else {
    print '<br /><b>Notice:</b> all passwords are stored in plaintext!';
  }
...
sub DoUpdatePrefs {
  my ($username, $password, $stylesheet, $pwlist);
  ...
  if ($password eq "") {
    ...
    $UserData{'password'} = &mirosCryptPW($password);
  }
  if (($AdminPass ne "") || ($EditPass ne "")) {
	...
	if ($EditPass eq "") {
	  ...
	  $pwlist = $AdminPass . " " . $EditPass;
	}
      }
      $UserData{'adminpw'} = &mirosEncodeUserIs($password, $pwlist);
      if (&UserIsAdmin()) {
  ...
  if (($uid > 199) && ($password ne "") && ($password ne "*")) {
    ...
    if ($UserID > 199) {
      if (defined($UserData{'password'}) &&
          (mirosCheckPW($password, $UserData{'password'}))) {
        $SetCookie{'id'} = $uid;
