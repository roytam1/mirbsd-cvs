...
#
#
#        a generic toolbox for sirc; to load it, type /load n0thing.pl
#
# version 2.5, last modified: 12 June 1997
# recent improvements:  lots of new shortcut aliases, removed ctcp pong
#			replies, added /pj and /wall (from wallop.pl),
#			moved the settings into SET variables, store saved
#			msgs in memory, toggleable re-join/re-umode on
#			reconnect, toggleable ignore of duplicated aways,
#			better unig, and opbeg becomes ctcp op #chn pass.

$add_ons.="+n0thing-v2.5.pl" if $add_ons !~ /n0thing/;
...
# settings; create the $set{} variables with their default values here.
...
# the autoop list, with fully-featured delayed and grouped auto-ops
...
# format of the entries in @n_autooplist :
# asked:channel:nick!user@host  in regexp format; asked is '.' or '1',
# channel is '.*' or a #channel name
...
sub cmd_sve {
  ...
  # old-style stuff for compatibility with old n0things
...
sub cmd_nhelp {
  &docommand("help n0thing".$args);
}
&addcmd("nhelp");

&addhelp("n0thing", "This is \cbn0thing.pl\cb for sirc, by \cborabidoo\cb
 
For the list of n0thing.pl's general functions, type /nhelp general
For the list of n0thing.pl's channel functions, type /nhelp channel
...
&addhelp("n0thinggeneral", "List of \cbn0thing.pl\cb's functions:

/a <message>            sends to the last person you /msg'd
/ver <nick>             gets version info about <nick>
/tim <nick>             gets <nick>'s local time
/setaway <reason>       sets yourself away, logs messages
/qsetaway <reason>      like /setaway without doing the /me
/setback                sets yourself back, shows messages
/clearmes               erases saved messages
/ig <nick>              ignores <nick> (using a user\@host mask)
/sig <nick>             ignores <nick>'s site
/unig [nick]            unignores <nick> or undoes your last /ig
/sve                    saves your settings and auto-op and notify list");

&addhelp("n0thingchannel", "List of \cbn0thing.pl\cb's channel functions:

/sb [<channel>]         shows ban list on a channel [current one]
/ban <nick> [<nick>...] bans on current channel
/sban <nick>            bans the whole site on current channel
/ub <nick>|<address>    unbans on current channel
/ub <number> [<nmbr>..] removes bans by number after a /sb
/cb                     clears all bans on current channel
/l                      short list of people on the current channel
/pj                     part and rejoin current channel
/open [<channel>]       removes weird modes on the channel [current one]
/kb <nick> [<reason>]   kicks and bans
/ksb <nick> [<reason>]  kicks and site-bans
/fk <filter> [<reasn>]  kicks all users matching the filter
/fkb <nick> [<reasn>]   site-bans and kicks all users from the same site
/wall <msg>             send a message to the current channel's ops");

&addhelp("n0thingsettings", 
"List of \cbn0thing.pl\cb's SET variables:
autorejoin [on|off]     toggles auto-rejoin on kick
rejoinchannels [on|off] toggles rejoining channels on reconnect
reconnect [on|off]      toggles auto-reconnect
awayonce [on|off]       toggles hiding duplicate away messages
slowserver [on|off]     toggles avoiding userhost on notify
wallv [on|off]          toggles sending walls to +v people
autoop [on|oppass|off]  toggle autoops (oppass requires a ctcp op w/ the
                        channel name and a password)
oppass <password>       toggles your opbeg password
quitmsg <message>       sets your default /quit message

List of \cbn0thing.pl\cb's setting-related commands:
/addop <aop> #chn <user> adds to op list; aop is 0 or 1; #chn can be *
/remop <nick>|<address> removes user from your auto-op list
/clearop                clears your auto-op list
/listop [#chn]          lists your auto-op list [for channel #chn]
/delay			sets your auto-op delay
/sve                    saves your settings and auto-op and notify list");

&addhelp("n0thingfiles", "List of \cbn0thing.pl\cb's file-oriented functions:
/dc [<nick>]            starts a dcc chat [last offered]
/dg [<nick>] [<file>]   gets a file via dcc [last offered]
/ds <nick> <file>       sends a file via dcc
/cdc <nick>             closes a /dcc chat
/cds <nick>             closes a /dcc send connection
/cdg <nick>             closes a /dcc get connection
/rdc <nick1> <nick2>    renames a /dcc chat
/ex <command>           \"shortcut\" for /system
/ls [<args>]            lists files
/cat <files>            shows files
/rm <files>             deletes files
/ps <options>           lists processes
/mail                   reads your mail  (you'll get disconnected from the
                        server if this takes more than a few minutes; exit
                        with 'q')");
...
&print("*\cb0\cb* \cborabidoo\cb's *\cvn0thing.pl\cv* for sirc loaded, type /nhelp for help");
