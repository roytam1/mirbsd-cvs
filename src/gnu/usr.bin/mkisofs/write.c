...
LOCAL	int	xawrite		__PR((void *buffer, int size, int count,
	...
	void	xfwrite		__PR((void *buffer, int size, int count,
					FILE *file, int submode, BOOL islast));
...
static	int	interpad_size	__PR((int starting_extent));
static	int	endpad_size	__PR((int starting_extent));
	...
	FILE	*outfile;
{
	 ...
	 */
	sprintf(iso_time, "%4.4lld%2.2d%2.2d%2.2d%2.2d%2.2d00",
		(int64_t)1900 + local.tm_year,
...
struct output_fragment startpad_desc	= {NULL, startpad_size,	NULL,	  startpad_write, "Initial Padbock"};
