		  ...
		  int recurse, enum val_prettyprint pretty)
{
  ...
  struct type *elttype;
  unsigned eltlen;
  int length_pos, length_size, capacity_pos, capacity_size, string_pos;
  int char_size;
  LONGEST val;
  CORE_ADDR addr;
  ...
  switch (TYPE_CODE (type))
    ...
    if (is_pascal_string_type (elttype, &length_pos, &length_size, &capacity_pos, &capacity_size,
	...
	{
    if (is_pascal_string_type (type, &length_pos, &length_size, &capacity_pos, &capacity_size,
                                     &string_pos, &char_size, NULL))
	    {
	      len = extract_unsigned_integer (valaddr + embedded_offset + length_pos, length_size);
        capacity = extract_unsigned_integer (valaddr + embedded_offset, length_size);
        if ((len < 0) || (len > capacity) || (capacity <= 0))
          {
            fprintf_filtered (stream, "<invalid data>");
            break;
          }
