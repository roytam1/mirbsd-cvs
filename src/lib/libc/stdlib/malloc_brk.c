/*	$OpenBSD: malloc.c,v 1.71 2004/08/11 06:22:45 tdeval Exp $	*/
 ...
 */
static int extend_pgdir(u_long indexx);
 ...
 */
static int
extend_pgdir(u_long indexx)
{
    struct  pginfo **new, **old;
    size_t i, oldlen;

    /* Make it this many pages */
    i = indexx * sizeof *page_dir;
    i /= malloc_pagesize;
    i += 2;
...
malloc_pages(size_t size)
{
	...
	indexx = ptr2index(p);
	page_dir[indexx] = MALLOC_FIRST;
	for (i=1;i<size;i++)
	    page_dir[indexx+i] = MALLOC_FOLLOW;
...
irealloc(void *ptr, size_t size)
{
    ...
    indexx = ptr2index(ptr);

    if (indexx < malloc_pageshift) {
	wrtwarning("junk pointer, too low to make sense\n");
	return (NULL);
    }

    if (indexx > last_index) {
...
free_pages(void *ptr, u_long indexx, struct pginfo *info)
{
    u_long i, l;
    ...
    page_dir[indexx] = MALLOC_FREE;
    for (i = 1; page_dir[indexx+i] == MALLOC_FOLLOW; i++)
	page_dir[indexx + i] = MALLOC_FREE;
	...
	indexx = ptr2index(pf->end);

	for(i=indexx;i <= last_index;)
...
ifree(void *ptr)
{
    ...
    info = page_dir[indexx];

    if (info < MALLOC_MAGIC)
        free_pages(ptr, indexx, info);
    else
	free_bytes(ptr, indexx, info);
    return;
}
