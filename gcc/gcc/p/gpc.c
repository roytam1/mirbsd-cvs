...
GNU Pascal is free software; you can redistribute it and/or modify it under
the terms of the GNU General Public License as published by the Free
Software Foundation; either version 2, or (at your option) any later
version.

GNU Pascal is distributed in the hope that it will be useful, but WITHOUT ANY
WARRANTY; without even the implied warranty of MERCHANTABILITY or
FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
for more details.

You should have received a copy of the GNU General Public License
along with GNU Pascal; see the file COPYING.  If not, write to the Free
...
#define ACONCAT(ARGS) (concat ARGS)
#define set_std_prefix(A, B)
...
#define bug_report_url "<URL:http://www.gnu-pascal.de/todo.html>"
...
#ifdef EGCS
/* The following code is from system.h of gcc-3.2.1 */
...
/* Say how to test for an absolute pathname.  On Unix systems, this is if
   ...
   filesystems, it is also absolute if it starts with a drive identifier.  */
...
#else
/* The config/... files don't set HAVE_DOS_BASED_FILE_SYSTEM yet. So
   let's kludge it a little. */
#define HAVE_DOS_BASED_FILE_SYSTEM (IS_DIR_SEPARATOR ('\\'))
#define IS_ABSOLUTE_PATHNAME(STR) \
  (IS_DIR_SEPARATOR ((STR)[0]) || (STR)[0] == '$' \
   || (IS_DIR_SEPARATOR ('\\') && (STR)[0] != '\0' && (STR)[1] == ':' && IS_DIR_SEPARATOR ((STR)[2])))
#endif
...
static const struct compiler default_compilers[] =
{
  ...
  {".F", "#Fortran", 0}, {".FOR", "#Fortran", 0}, {".FPP", "#Fortran", 0},
  {".r", "#Ratfor", 0},
  {".pas", "#Pascal", 0}, {".p", "#Pascal", 0}, {".pp", "#Pascal", 0}, {".dpr", "#Pascal", 0},
  {".java", "#Java", 0}, {".class", "#Java", 0},
  {".zip", "#Java", 0}, {".jar", "#Java", 0},
  /* Next come the entries for C.  */
  {".c", "@c", 0},
  {"@c",
#if defined (EGCS97) || defined (USE_CPPLIB)
...
translate_options (int *argcp, const char *const **argvp)
{
  ...
  while (i < newindex)
    {
      /* If they only gave us `-v' or `-h', don't try to link libgpc. */
      if (library != 0
          && newv[i][0] == '-'
          && (newv[i][1] == 'h' || newv[i][1] == 'v')
          && newv[i][2] == '\0'
          && (newindex == 2
              || (newindex == 3
...
static struct spec_list static_specs[] =
{
  ...
  INIT_STATIC_SPEC ("multilib_extra",           (const char **) &multilib_extra),
...
static int warn_std ATTRIBUTE_UNUSED;
...
add_preprocessor_option (const char *option, int len)
{
  ...
  if (! preprocessor_options)
    preprocessor_options = (char **) xmalloc (n_preprocessor_options * sizeof (char *));
  else
...
add_assembler_option (const char *option, int len)
{
  ...
  if (! assembler_options)
    assembler_options = (char **) xmalloc (n_assembler_options * sizeof (char *));
  else
...
add_linker_option (const char *option, int len)
{
  ...
  if (! linker_options)
    linker_options = (char **) xmalloc (n_linker_options * sizeof (char *));
  else
...
#ifdef GPC
  GET_ENVIRONMENT (gcc_exec_prefix, "GPC_EXEC_PREFIX");
  if (! gcc_exec_prefix)
#endif
...
#ifdef GPC
  if (argc == 1 || (argc == 2 && strncmp (argv[1], "-famtmpfile=", 12) == 0))
...
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n\n"),
      ...
      else if (strcmp (argv[i], "-Xassembler") == 0)
	{
	  ...
	  add_assembler_option (argv[i+1], strlen (argv[i+1]));
        ...
        {
	  struct user_specs *user = (struct user_specs *) xmalloc (sizeof (struct user_specs));
          if (++i >= argc)
      ...
      else if (argv[i][0] == '-' && argv[i][1] != 0)
        {
          ...
          switch (c)
            ...
            case 'o':
              ...
              if (p[1] == 0)
                ((const char **) argv)[i + 1] = convert_filename (argv[i + 1], ! have_c, 0);
              else
                ((const char **) argv)[i] = convert_filename (argv[i], ! have_c, 0);
#endif
              goto normal_switch;
  ...
  switches = (struct switchstr *) xmalloc ((n_switches + 1) * sizeof (struct switchstr));
      ...
      else if (strcmp (argv[i], "-Xpreprocessor") == 0)
	{
	  ...
	  infiles[n_infiles++].name = argv[++i];
		...
		= (const char **) xmalloc ((n_args + 1) * sizeof(const char *));
	      ...
	      switches[n_switches].args = (const char **) xmalloc (2 * sizeof (const char *));
    ...
    switch (inswitch ? 'a' : c)
      ...
      case '%':
        switch (c = *p++)
          {
          case 0:
            fatal ("Invalid specification! Bug in gpc.");
	  ...
	  case 'V':
	    ...
	    break;
          ...
          /* The following case comes from the Cygwin/mingw
             gcc-2.95.3 patches. It shouldn't hurt anyone else. */
          case 'q':
            {
              char *prefix;
              char *incprefix;
              const char *oprefix = p;
              int prefix_length;
              oprefix = p;
              while (*p && ! ISSPACE ((unsigned char) *p))
                p++;
              prefix_length = p - oprefix;
              prefix = xmalloc (prefix_length + 1);
              strncpy (prefix, oprefix, prefix_length);
              prefix[prefix_length] = '\0';
              /* the GCC patches have `&warn_B' instead of the latter `NULL', but this causes spurious warnings, tc20020309 */
              add_prefix (&exec_prefixes, prefix, NULL, 1, 0, NULL, 0);
              add_prefix (&startfile_prefixes, prefix, NULL,
                          PREFIX_PRIORITY_B_OPT, 0, NULL, 0);
              incprefix = concat (prefix, "include", NULL);
              add_prefix (&include_prefixes, incprefix, NULL,
                          PREFIX_PRIORITY_B_OPT, 0, NULL, 0);
              free (incprefix);
              free (prefix);
              break;
  ...
  for (endp = p; *endp != '\0'; endp++)
    {
      ...
      if (!ISALNUM ((int) *endp) && !(*endp == '-' || *endp == '_'))
...
main (int argc, const char **argv)
{
...
#ifdef GPC
  library = 2;
  init_gpc_options (&argc, (const char ***) &argv);
#endif
  ...
  /* Build multilib_select, et. al from the separate lines that make up each
     multilib selection.  */
  {
    const char *const *q = (const char **) multilib_raw;
    int need_space;
    ...
    q = (const char **) multilib_matches_raw;
    while ((p = *q++) != (char *) 0)
      obstack_grow (&multilib_obstack, p, strlen (p));
  ...
  if (verbose_flag)
    {
      ...
      if (! strncmp (version_string, compiler_version, n)
        ...
        notice ("gpc version %s, based on gcc-%s\n", GPC_RELEASE_STRING, version_string);
      else
        notice ("gpc driver version %s, based on gcc-%s, executing gpc version %s\n",
                GPC_RELEASE_STRING, version_string, compiler_version);
  ...
  if (n_mdswitches)
    {
      ...
      mdswitches = (struct mdswitchstr *) xmalloc (sizeof (struct mdswitchstr) * n_mdswitches);
...
init_gpc_options (int *argcp, const char ***argvp)
{
  ...
  for (i = 1; i < argc; i++)
    {
      if (!strncmp (argv[i], "-famtmpfile", 11) || !strncmp (argv[i], "--amtmpfile", 11))
        amt = 1;
      /* `--interface-only' implies `--syntax-only' for all purposes,
         including further specs processing. That's why we add it here. */
      if (!strcmp (argv[i], "-finterface-only") || !strcmp (argv[i], "--interface-only"))
        new_argv[new_argc++] = "-fsyntax-only";
    }
  if (!amt)
    {
      /* Choose a name for the automake temp file. */
      automake_temp_file_name = make_temp_file (".gpa");
      new_argv[new_argc++] = concat ("-famtmpfile=", automake_temp_file_name, NULL);
    }
  if (!new_argc)
    return;
  argv = (const char **) xmalloc ((argc + new_argc + 1) * sizeof (const char *));
  argv[0] = (*argvp)[0];
  memcpy (argv + 1, new_argv, new_argc * sizeof (const char *));
  memcpy (argv + new_argc + 1, *argvp + 1, (argc - 1) * sizeof (const char *));
...
init_automake_temp_file (int argc, const char *const *argv)
{
  ...
  fprintf (automake_temp_file, "#pid: %i\n", (int) getpid ());
  fprintf (automake_temp_file, "#cmdline:");
  for (i = 1; i < argc; i++)
    {
      int j, ok = strcmp (argv[i], "-E") != 0
                  && strcmp (argv[i], "-c") != 0
                  && strcmp (argv[i], "-S") != 0
                  && !(strncmp (argv[i], "-x", 2) == 0
                       && strcmp (argv[i], "-xPreprocessed-Pascal") != 0
                       && !(i + 1 < argc && strcmp (argv[i + 1], "Preprocessed-Pascal") == 0))
                  ...
                  && !(strncmp (argv[i], "--automake", 10) == 0 && argv[i][10] != '-')
                  && !(strncmp (argv[i], "-fautomake", 10) == 0 && argv[i][10] != '-')
                  && strncmp (argv[i], "--autobuild", 11) != 0
                  && strncmp (argv[i], "-fautobuild", 11) != 0
                  && strncmp (argv[i], "-famtmpfile", 11) != 0;
      for (j = 0; j < n_infiles && ok; j++)
        if (strcmp (argv[i], infiles[j].name) == 0)
          ok = 0;
      if (ok)
        fprintf (automake_temp_file, " %s", quote_arg (argv[i]));
      /* With `-x' and `-o', do not pass the given language or file name either. */
      else if (strcmp (argv[i], "-x") == 0 || strcmp (argv[i], "-o") == 0)
        i++;
    }
...
check_amtmpfile_switch (void)
{
  ...
  for (i = 0; i < n_infiles; i++)
    {
      const char *n = infiles[i].name, *p = n + strlen (n);
      while (p != n && *p != '.') --p;
      if (strcmp (p, ".pas") == 0 || strcmp (p, ".p") == 0
          || strcmp (p, ".pp") == 0 || strcmp (p, ".dpr") == 0
          || (infiles[i].language
              && (!strcmp (infiles[i].language, "Pascal")
                  || !strcmp (infiles[i].language, "Preprocessed-Pascal"))))
        return;
    }
  ...
  while (!feof (automake_temp_file))
    {
      ...
      if (i >= n_infiles)
        {
          ...
          const char **new_outfiles = (const char **) xmalloc (new_n_infiles *
                                                               sizeof (char *));
          char *new_explicit_link_files = xmalloc (new_n_infiles);
          memcpy ((char *) new_outfiles,
                  (char *) outfiles,
                  (n_infiles) * sizeof (char *));
          memcpy ((char *) new_explicit_link_files,
                  (char *) explicit_link_files,
                  n_infiles);
      ...
      if (fgets (buffer, 10240, automake_temp_file) == NULL)
        break;
      l = strlen (buffer);
      if (l > 0 && buffer[l - 1] == '\n')
        buffer[l - 1] = 0;
      ...
      /* Process an implicit `-o' switch, but only if no explicit one is present. */
...
make_temp_file (const char *suffix)
{
  ...
  if (fd == -1)
    abort ();
  /* Similarly if we cannot close the file.  */
  if (close (fd))
    abort ();
